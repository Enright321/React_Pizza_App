...
const MenuItemEditScreen = ({ match, history }) => {
  ...
  useEffect(() => {
    if (successUpdate) {
      ...
      history.push('/admin/menuItemlist');
    } else {
      if (!menuItem.name || menuItem._id !== menuItemId) {
  ...
  }, [dispatch, history, menuItem, menuItemId, successUpdate]);
  ...
  return (
    <>
      <Link to='/admin/menuitemlist' className='btn btn-light my-3'>
        Go Back
      </Link>
      <FormContainer>
